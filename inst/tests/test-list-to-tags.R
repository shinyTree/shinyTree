...
test_that("named list works", {  
  ...
  expect_equal(listToTags(li), tags$ul(tags$li("abc"), tags$li("def")))
})
...
test_that("opened attributes work", {
  li <- structure(list(a=1, b=2), jtopened=TRUE)
...
test_that("nested opened attributes work", {
  li <- list(abc = "hi", def=structure(list(a=1, b=2), jtopened=TRUE))
  expect_equal(listToTags(li), 
...
test_that("selected attributes work", {
  li <- structure(list(a=1, b=2), jtselected=TRUE)
  expect_equal(listToTags(li),
...
test_that("disabled attributes work", {
  li <- structure(list(a=1, b=2), jtdisabled=TRUE)
  expect_equal(listToTags(li),
...
test_that("multiple modifications work", {
  li <- structure(list(a=1, b=2), jtopened=TRUE, jtselected=TRUE, jtdisabled=TRUE)
  expect_equal(listToTags(li),
